...
void CModeConv::putYSF_Mode1(unsigned char* data, FILE *file) {
	...
	for (unsigned int j = 0U; j < 5U; j++, offset += 144U) {
		...
		unsigned int dat_a = CGolay24128::decode24128(a);  //128
		// The PRNG
		b ^= PRNG_TABLE[dat_a] >> 1;
		unsigned int dat_b = CGolay24128::decode23127(b);  // 127(b>>1)
    ...
    if (file) {
		...
		//CUtils::dump(1U, "VCH V/D type 1:", buf, 40U);
	}	
}
