...
const unsigned char dt1_temp[] = {0x31, 0x22, 0x62, 0x5F, 0x20, 0x00, 0x00, 0x00, 0x00, 0x00};
const unsigned char dt2_temp[] = {0x00, 0x00, 0x00, 0x00, 0x6C, 0x20, 0x1C, 0x20, 0x03, 0xFF};
//const unsigned char dt1_nogps[] = {0x31, 0x22, 0x61, 0x5F, 0x20, 0x00, 0x00, 0x00, 0x00, 0x00};
//const unsigned char dt2_nogps[] = {0x00, 0x00, 0x00, 0x00, 0x6C, 0x20, 0x1C, 0x20, 0x03, 0xFE};
const unsigned char dt1_nogps[]   = {0x01, 0x22, 0x61, 0x5F, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20};
const unsigned char dt2_nogps[]   = {0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x03, 0xA6};
...
void CAPRSReader::CrcGPS(unsigned char *buffer) {
	//crc calculation
	int i;
	int crc = 0;
	...
	while ((nDataLength = sockfd.read(buffer, 10000, APRS_TIMEOUT)) > 0){
		...
		for (i = 0; i < nDataLength; i++) {
			if ((i > 3) && buffer[i - 4] == '\"' && buffer[i - 3] == 'l' && buffer[i - 2] == 'a' && buffer[i - 1] == 't' && buffer[i] == '\"'){
				...
				if (strspn(tmp_str, " -0123456789.") == strlen(tmp_str)) {
...
bool CAPRSReader::findCall(std::string cs, int *latitude, int *longitude)
{
	...
	if (not_found) {
		...
		//	LogMessage("Location expired time: epoch:%d, refresh:%d",epoch,m_refres_time);
			m_new_callsign = true;
			m_cs = cs;
		}
...
void CAPRSReader::get_gps_buffer(unsigned char *buffer, std::string callsign){
int lat, lon;	
	...
	if (!callsign.empty() && findCall(callsign, &lat, &lon)) {
			formatGPS(buffer, lat, lon);
	} else {
	::memcpy(buffer, dt1_nogps, 10U);
	::memcpy(buffer + 10U, dt2_nogps, 10U);
	buffer[0]=m_gps_buffer_cnt%0xFF;
	m_gps_buffer_cnt++;
	CrcGPS(buffer);
	}
